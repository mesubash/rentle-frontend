...
export function ListingsManager() {
  ...
  return <main className="page"><div className={`container ${styles.page}`}>
    ...
    {loading ? <div className={styles.grid}>{[1, 2, 3, 4].map((item) => <div className="skeleton" key={item} />)}</div>
      : error ? <section className="empty-state card"><h2>Could not load listings</h2><p>{error}</p></section>
      ...
      : <section className="empty-state card"><h2>You have no listings yet.</h2><p>Create one with real details and current photos.</p><Link className="button" href="/list">Create your first listing</Link></section>}
