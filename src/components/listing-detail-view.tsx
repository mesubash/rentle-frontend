...
export function ListingDetailView({ listingId }: { listingId: string }) {
  ...
  return <><main className="page listing-detail-page"><div className="container">
    ...
    <div className="listing-detail-layout"><article className="listing-content">
      ...
      {listing.rentalTerms && <section className="detail-section"><h2>Rental terms</h2><p style={{ whiteSpace: "pre-wrap" }}>{listing.rentalTerms}</p></section>}
      ...
      <section className="owner-card card"><div className="owner-card__head"><span className="avatar avatar--large">{initials(listing.owner.businessName || listing.owner.fullName)}</span><div><p className="eyebrow">Hosted by</p><h2>{listing.owner.businessName || listing.owner.fullName}</h2><div className="owner-card__badges"><TrustBadge verified={listing.owner.verified} />{listing.owner.accountType === "BUSINESS" && <span className="business-badge"><ShieldCheck size={13} /> Registered business</span>}</div></div></div><dl><div><dt>Trust score</dt><dd>{formatTrustScore(listing.owner.trustScore)}</dd></div><div><dt>Listing reviews</dt><dd>{listing.reviewCount}</dd></div><div><dt>Member since</dt><dd>{joined}</dd></div><div><dt>Completed bookings</dt><dd>{listing.totalBookings}</dd></div></dl><Link className="button button--secondary button--small" href={`/profile/${listing.owner.id}`}><MessageCircle size={16} /> View profile</Link></section>
