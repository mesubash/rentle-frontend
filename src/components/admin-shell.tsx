...
export function AdminShell({ children }: { children: React.ReactNode }) {
  ...
  if (!user || user.role !== "ADMIN") return <main className="page"><div className="container narrow-page"><section className="empty-state card"><h1>Admin access required</h1><p>This workspace is available only to Rentle administrators.</p><Link className="button" href="/auth/login">Log in with an admin account</Link></section></div></main>;
