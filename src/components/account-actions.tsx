...
export function AccountActions() {
  ...
  async function signOut() { setLeaving(true); try { await logout(); showToast("You have been logged out safely.", { tone: "success" }); } catch { showToast("You were logged out on this device, but the server could not be reached.", { tone: "info" }); } finally { router.push("/"); router.refresh(); } }
