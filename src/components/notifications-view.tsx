...
export function NotificationsView() {
  const { user } = useAuth(); const [renting, setRenting] = useState<Booking[]>([]); const [hosting, setHosting] = useState<Booking[]>([]); const [error, setError] = useState("");
  useEffect(() => { Promise.all([bookingsApi.asRenter(0,50), bookingsApi.asOwner(0,50)]).then(([renter, owner]) => { setRenting(renter.content); setHosting(owner.content); }).catch(() => setError("Activity could not be loaded.")); }, []);
  const actions = [
