...
export function BookingPanel({ listing }: { listing: ListingDetail }) {
  ...
  function openSheet() {
    if (isOwner) {
      router.push(`/listings/manage/${listing.id}`);
      return;
    }
    if (!user) {
      if (loading) return;
  ...
  return <>
    <aside className="booking-panel"><p className="booking-panel__price"><strong>{formatNpr(listing.pricePerUnit)}</strong> / {unit}</p><div className="booking-panel__deposit"><ShieldCheck size={17} /><span><small>Refundable security deposit</small><strong>{formatNpr(listing.depositAmount)}</strong></span></div><button className="button button--wide" onClick={openSheet}>{isOwner ? "Manage listing" : isService ? "Check availability" : "Request booking"}</button><p className="booking-panel__fine">{isOwner ? "Update availability, photos, pricing, and status." : "The owner reviews your dates before any deposit is due."}</p></aside>
    <div className="mobile-booking-bar"><p><strong>{formatNpr(listing.pricePerUnit)}</strong><small> / {unit}</small></p><button className="button" onClick={openSheet}>{isOwner ? "Manage listing" : isService ? "Check date" : "Request booking"}</button></div>
